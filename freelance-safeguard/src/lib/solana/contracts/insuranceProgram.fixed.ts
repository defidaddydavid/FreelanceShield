...
export class InsuranceProgram {
  ...
  constructor(
    ...
    programId: PublicKey = PROGRAM_IDS.INSURANCE_PROGRAM
  ) {
    ...
    this.program = new anchor.Program(
      require('../idl/insurance_program.json'),
      programId,
      provider
    );
  }
   ...
   * Cancel a policy
   */
  async cancelPolicy(policyAddress: PublicKey): Promise<string> {
    try {
      // Derive the insurance state PDA
      const [insuranceStatePda] = await PublicKey.findProgramAddress(
        [Buffer.from('insurance_state')],
